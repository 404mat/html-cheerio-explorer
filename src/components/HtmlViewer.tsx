...
const HtmlViewer: React.FC<HtmlViewerProps> = ({ html, isLoading }) => {
  ...
  useEffect(() => {
    ...
    if (filters.includes("head")) {
      processedHtml = processedHtml.replace(/<head\b[^>]*>[\s\S]*?<\/head>/gi, '<!-- <head> content hidden -->');
    }
    
    if (filters.includes("scripts")) {
      processedHtml = processedHtml.replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi, '<!-- <script> content hidden -->');
    }
    
    if (filters.includes("styles")) {
      processedHtml = processedHtml.replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi, '<!-- <style> content hidden -->');
